...
try{
 await check('health and game config identify the correct game',async()=>{
  const r=await fetch(base+'/api/config'),j=await r.json();assert.equal(j.version,'38.0.0');assert.equal(j.physicsVersion,72);assert.equal(j.cards,73);assert.equal(j.units,65);assert.equal(j.spells,8);assert.equal(j.maxDeck,8);assert.equal(j.game,'tiny-siege');
 });
