...
try{
 await check('health and game config identify the correct game',async()=>{
  const r=await fetch(base+'/api/config'),j=await r.json();assert.equal(j.version,'37.2.0');assert.equal(j.physicsVersion,63);assert.equal(j.cards,66);assert.equal(j.units,59);assert.equal(j.spells,7);assert.equal(j.maxDeck,8);assert.equal(j.game,'tiny-siege');
 });
 ...
 await check('validated deployment is synchronized to both players',async()=>{
  ...
  a.send({type:'deploy',card:id,x:140,y:800});
 ...
 await check('server sends v37.2.0 / physics-v63 grid deployment and combat metadata identically to both seats',async()=>{
  const ga=a.last().game,u=ga.units[0];assert.equal(ga.physicsVersion,63);assert.ok(['grass','stone','lava','snow','desert'].includes(ga.mapTheme));assert.equal(typeof u.facing,'number');assert.equal(typeof u.firstStrikeRemaining,'number');assert.equal(typeof u.mass,'number');assert.equal(typeof u.deploying,'boolean');assert.equal(typeof u.targetable,'boolean');assert.equal(typeof u.deployRemaining,'number');assert.equal(typeof u.shieldHp,'number');assert.equal(typeof u.maxShieldHp,'number');assert.equal(typeof u.stealthed,'boolean');assert.equal(typeof u.stealthRemaining,'number');assert.equal(typeof u.eggHatchRemaining,'number');assert.equal(typeof u.revived,'boolean');assert.equal(typeof u.drillStage,'number');assert.equal(typeof u.drillDps,'number');assert.equal(typeof u.drillLockTime,'number');assert.equal(typeof u.crusherStage,'number');assert.equal(typeof u.turtleShellActive,'boolean');assert.ok(Object.hasOwn(u,'megaJumpState'));assert.equal(typeof u.megaJumpProgress,'number');assert.ok(Object.hasOwn(u,'riverJumpMode'));assert.ok(Object.hasOwn(u,'riverJumpState'));assert.equal(typeof u.riverJumpProgress,'number');assert.equal(typeof u.ironSpinUsed,'boolean');assert.ok(Object.hasOwn(u,'ironSpinState'));assert.equal(typeof u.ironMarkProgress,'number');assert.ok(Object.hasOwn(u,'hookState'));assert.equal(typeof u.hookProgress,'number');assert.equal(typeof u.hookCooldownRemaining,'number');assert.equal(typeof u.hookAirAttackRemaining,'number');
