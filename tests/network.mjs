...
try{
 await check('health and game config identify the correct game',async()=>{
  const r=await fetch(base+'/api/config'),j=await r.json();assert.equal(j.version,'18.1.0');assert.equal(j.physicsVersion,17);assert.equal(j.cards,30);assert.equal(j.units,26);assert.equal(j.spells,4);assert.equal(j.maxDeck,8);assert.equal(j.game,'tiny-siege');
 });
 ...
 await check('server sends v18 / physics-v17 facing, summon, stun, charge, mud, burrow, poison, dash, laser, tower-awake and collision metadata identically to both seats',async()=>{
  const ga=a.last().game,u=ga.units[0];assert.equal(ga.physicsVersion,17);assert.equal(typeof u.facing,'number');assert.equal(typeof u.mass,'number');
