...
try{
 await check('health and game config identify the correct game',async()=>{
  const r=await fetch(base+'/api/config'),j=await r.json();assert.equal(j.version,'36.0.0');assert.equal(j.physicsVersion,57);assert.equal(j.cards,66);assert.equal(j.units,59);assert.equal(j.spells,7);assert.equal(j.maxDeck,8);assert.equal(j.game,'tiny-siege');
 });
