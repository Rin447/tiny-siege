...
test('worker health and config identify twenty-one units plus three spells custom-deck game',async()=>{
 const r=await worker.fetch(req('/api/config'),env());const b=await r.json();assert.equal(b.version,'15.0.0');assert.equal(b.cards,24);assert.equal(b.units,21);assert.equal(b.spells,3);assert.equal(b.physicsVersion,13);assert.equal(b.maxDeck,8);assert.equal(b.game,'tiny-siege');
});
