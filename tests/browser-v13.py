...
async def main():
    ...
    async with async_playwright() as p:
        ...
        text=await page.locator('#patchNotes').inner_text();assert 'TARGET LOCK' in text and 'v13.0.0' in text
        await page.screenshot(path=str(OUT/'00-v13-patch-notes.png'),full_page=True)
        ...
        await ok('v13 deck editor exposes twenty cards, eight-card selection, and average cost')
        await page.screenshot(path=str(OUT/'01-v13-deck-editor.png'),full_page=True)
        ...
        await page.screenshot(path=str(OUT/'02-v13-card-library.png'),full_page=True)
        ...
        await page.screenshot(path=str(OUT/'03-v13-cpu-battle.png'),full_page=True)
        await ok('CPU battle keeps four-card hand; cannon range preview is available when the card is drawn')
        assert not errors,errors
        await ok('no uncaught JavaScript errors in v13 browser flow')
        await browser.close()
    ...
    (ROOT/'docs/BROWSER_V13_RESULTS.json').write_text(json.dumps(report,indent=2,ensure_ascii=False))
