...
test('v35 physics version and mass/radius data are explicit',()=>{
 assert.equal(VERSION,'36.0.0');assert.equal(PHYSICS_VERSION,57);
 assert.ok(UNITS.knight.mass>UNITS.archer.mass);assert.ok(UNITS.knight.radius>UNITS.archer.radius);
 assert.equal(createMatch().physicsVersion,57);
});
...
test('server snapshots expose facing/mass/layer but not pathfinding internals',()=>{
 const g=game(),u=unit(g,'knight',0,190,930);advance(g,15);const snap=viewMatch(g,0);assert.equal(snap.physicsVersion,57);
