...
export class RoomModel {
  ...
  receive(id,raw){
    ...
    if(m.type==='ready'){
      ...
      if(m.ready){const deck=validateDeck(m.deck);if(!deck){this.error(c,'デッキは重複なしの8枚を選んでください。');return;}p.deck=deck;}
    ...
    if(m.type==='start'){
      ...
      if(!this.data.players.every(v=>v?.connected&&v.ready&&validateDeck(v.deck))){this.error(c,'2人とも8枚のデッキを決めて準備OKにしてください。');return;}
