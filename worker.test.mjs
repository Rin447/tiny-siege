...
test('worker health and config identify fifty units plus six spells custom-deck game',async()=>{
 const r=await worker.fetch(req('/api/config'),env());const b=await r.json();assert.equal(b.version,'29.0.0');assert.equal(b.cards,56);assert.equal(b.units,50);assert.equal(b.spells,6);assert.equal(b.physicsVersion,45);assert.equal(b.maxDeck,8);assert.equal(b.game,'tiny-siege');for(const feature of ['shield-front-absorb','wind-knockback','phoenix-egg-revive','gravity-pull','mirage-stealth','cyclone-field-pull','sky-bomber-all-targets','scrap-drill-ramping','crusher-ogre-ramping','siege-turtle-ranged-armor','bomb-carrier-suicide','iron-eye-mark-spin','tracker-hook','valkyrie-spin','gargoyle-three','gargoyle-swarm-six','global-move-speed-5-up-from-v26-1','valkyrie-axe-turn','leaf-archer-duo','iron-guard-damage-202','kragg-berserker-7-3760-842','mini-berserker-1390-755-1-6','leaf-archer-cost-3','golem-4256-260-2-5','mini-golem-one-fifth','golem-arm-sway','leaf-archer-duo-deploy-preview','tower-balance-4560-3200','battle-card-damage-hold','cannon-1000-200-1s','bat-horde-five-92-82-1-2','bomber-cost-2-arc-bomb','mega-knight-3993-263-jump-537','tombstone-skeleton-spawner','skeleton-squad-fifteen-81','skeleton-three-1-cost-81','iron-boar-1696-318-1-6-river-jump','riverbank-deploy-snap','healer-on-hit-110-three-allies','elixir-golem-split-energy','royal-giant-cannon-165','necromancer-5-839-1-1','dark-necromancer-4-907-304','poison-8s-91-21','spell-balance-fireball-689-159-arrowrain-366-75','lightning-top-four-1056-265','elixir-golem-demo-split-visual'])assert.ok(b.features.includes(feature),feature);
});
